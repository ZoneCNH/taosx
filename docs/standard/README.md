...
- [供应链与 Evidence](../supply-chain.md)：workflow Action SHA pinning、生产 workflow 强制 `govulncheck` 固定版本、manifest 校验和 CI artifact 对齐。
...
CI、Release Check 和 Security workflow 的第三方 Action 必须 pin 到 40 位 commit SHA，并保留来源 tag 注释。生产发布相关 workflow 必须固定安装 `golang.org/x/vuln/cmd/govulncheck@v1.1.4` 并通过 `XLIB_ENABLE_VULNCHECK=1`、`XLIB_FORCE_VULNCHECK=1` 强制执行漏洞扫描；本地 `make security` 仍按 weekly/force 开关判断是否运行漏洞库扫描。release manifest 测试必须在临时 fixture 仓库构造 `.omc` state，不得读取当前工作区的 Agent 运行态。
