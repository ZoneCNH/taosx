...
生产发布相关 workflow 必须安装固定版本 `golang.org/x/vuln/cmd/govulncheck@v1.1.4`，并显式设置 `XLIB_ENABLE_VULNCHECK=1` 与 `XLIB_FORCE_VULNCHECK=1`，覆盖本地 weekly 窗口以强制执行漏洞扫描。Docker toolchain image 可以安装固定版本以保证工具链可复现；本地入口仍由 `XLIB_ENABLE_VULNCHECK`、一周窗口状态和 `XLIB_FORCE_VULNCHECK` 控制。当前 Docker toolchain lint 基线是 `golangci-lint@v2.1.6`，workflow gate 使用固定安装的 `golangci-lint@v2.6.2`；升级时应同步更新 Dockerfile、workflow、发布文档和验证记录。
